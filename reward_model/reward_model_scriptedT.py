            ...
            if full_start < len(obs):
                ...
                for idx in range(len(full_obs) // batch_size + 1):
                    ...
                    if start < len(full_obs):
                        ...
                        dist = torch.norm(
                            obs[full_start:full_end, None, :].to(device)
...
class RewardModel:
    ...
    def get_label(self, sa_t_1, sa_t_2, r_t_1, r_t_2):
        ...
        if self.teacher_beta > 0:  # Bradley-Terry rational model
            r_hat = torch.cat(
                [torch.Tensor(sum_r_t_1), torch.Tensor(sum_r_t_2)], axis=-1
            )
    ...
    def kcenter_sampling(self):
        ...
        temp_sa = np.concatenate(
            [temp_sa_t_1.reshape(num_init, -1), temp_sa_t_2.reshape(num_init, -1)],
            axis=1,
        )
        ...
        tot_sa = np.concatenate(
            [tot_sa_1.reshape(max_len, -1), tot_sa_2.reshape(max_len, -1)], axis=1
        )
