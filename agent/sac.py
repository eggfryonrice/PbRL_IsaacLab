...
class SACAgent(Agent):
    ...
    def __init__(
        ...
        self.s_ent_stats = my_utils.TorchRunningMeanStd(shape=[1], device=device)
    ...
    def reset_actor(self, actor_cfg):
        # reset log_alpha
    ...
    def update_state_ent(self, replay_buffer, logger, step, gradient_update=1, K=5):
        for index in range(gradient_update):
            obs, full_obs, action, reward, next_obs, not_done, not_done_no_max = (
                replay_buffer.sample_state_ent(self.batch_size)
            )
